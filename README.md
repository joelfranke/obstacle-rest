...
## Pre-event check
`npm run test:event:local` registers a small field, starts them, posts obstacle results, and checks individual and team scores against the local API. It covers the nested dashboard scoring routes (`/scoring/participants`, `/scoring/teams`, `/scoring/results`) and the query-style routes the iOS apps actually call (`/scoring?…`, `/participant`, `/results/:bibNo`, `/heats`, `/teams`, `/timing`). Writes use the JSON bodies compiled by Check In and Recorder. MongoDB must already be running at `mongodb://127.0.0.1:27017`. If no admin token is stored, the script seeds one and prints it. Dev (`npm run test:event:dev`) runs the same day and needs `API_TOKEN`. Prod (`npm run test:event:prod`) only checks that the API is up and that unauthenticated writes are rejected.
